...
import { createVariedad, updateVariedad, deleteVariedad } from './variedadesActions'
import type { Variedad } from './variedadesActions'

interface VariedadesManagerProps {
  variedades: Variedad[]
}

export default function VariedadesManager({ variedades }: VariedadesManagerProps) {
  ...
  function startEditing(v: Variedad) {
    setEditingId(v.id)
    setEditNombre(v.nombre)
    setEditAbreviatura(v.abreviatura)
    setError(null)
  }
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (!newNombre.trim()) {
      setError('El nombre de la variedad es requerido.')
      return
    }
    ...
    startTransition(async () => {
      const res = await createVariedad(fd)
      if (!res.success) {
        setError(res.error || 'Error al crear variedad.')
      } else {
        setNewNombre('')
        setNewAbreviatura('')
  ...
  async function handleUpdate(e: React.FormEvent, id: number) {
    ...
    startTransition(async () => {
      const res = await updateVariedad(fd)
      if (!res.success) {
        setError(res.error || 'Error al actualizar variedad.')
      } else {
        setEditingId(null)
      }
    })
  ...
  async function handleDelete(id: number) {
    setError(null)
    if (!confirm('¿Estás seguro de que deseas eliminar esta variedad? El cambio no afectará a los lotes históricos pero impedirá su uso futuro.')) return
  ...
  const filteredVariedades = variedades.filter(v => 
    ...
    v.abreviatura.toLowerCase().includes(searchQuery.toLowerCase())
  )
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
        ...
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Buscar variedad o abreviatura..."
            value={searchQuery}
        ...
        >
          {showAddForm ? '✕ Cancelar' : '＋ Nueva Variedad'}
        </button>
      </div>
      ...
      {showAddForm && (
        <form onSubmit={handleCreate} className="bg-[#1a120b]/60 backdrop-blur-xl rounded-3xl border border-[#c2a077]/30 shadow-2xl p-6 space-y-4">
          <h3 className="text-lg font-bold text-[#c2a077]">Agregar Nueva Variedad</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs uppercase tracking-wider text-gray-400 mb-1.5">Variedad *</label>
              <input
                type="text"
                value={newNombre}
          ...
          <div className="flex justify-end pt-2">
            ...
            >
              {isPending ? 'Guardando...' : 'Crear Variedad'}
            </button>
          </div>
        </form>
      )}
          ...
          <table className="w-full text-left border-collapse min-w-[600px]">
            <thead>
              <tr className="bg-white/5 text-[#c2a077] text-sm tracking-wider uppercase">
                <th className="p-5 font-semibold">Variedad</th>
            ...
            <tbody className="divide-y divide-white/5">
              {filteredVariedades.map((v) => {
                ...
                if (isEditing) {
                  return (
                    <tr key={v.id} className="bg-white/[0.02]">
                      <td colSpan={3} className="p-4">
                        <form onSubmit={(e) => handleUpdate(e, v.id)} className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 w-full">
                          <div className="flex-1 min-w-[200px]">
                            <label className="block text-[10px] text-gray-400 mb-1">Variedad *</label>
                            <input
                              type="text"
                              value={editNombre}
                ...
                return (
                  <tr key={v.id} className="hover:bg-white/[0.03] transition-colors duration-200 group">
                    <td className="p-5">
                      <span className="text-white font-bold uppercase block">{v.nombre}</span>
                    </td>
                    <td className="p-5">
